...
def prepare_test_loader(config: dict[str, Any]) -> DataLoader:
    ...
    x_test, y_test, test_lengths = processor.prepare_tensors(
        processor.test_raw,
        return_lengths=True,
    )
    test_ds = TensorDataset(x_test.cpu(), test_lengths.cpu(), y_test.cpu())
    return DataLoader(test_ds, batch_size=training_cfg["batch_size"], shuffle=False)


def evaluate_seed(seed: int) -> dict[str, Any]:
    config_path = Path("outputs") / f"lstm_w2vec_seed{seed}_config.yaml"
    model_path = Path("models") / f"lstm_w2vec_seed{seed}_best_model.pt"
    embedding_matrix_path = Path("models") / f"lstm_w2vec_seed{seed}_embedding_matrix.pt"
    ...
    embedding_matrix = torch.load(embedding_matrix_path, map_location="cpu")
...
def run_test_seed_evaluation(seeds: list[int] | None = None) -> dict[str, Any]:
    ...
    summary_path = report_dir / "lstm_w2vec_test_seed_summary.yaml"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate LSTM Word2Vec seed models on test split.")
