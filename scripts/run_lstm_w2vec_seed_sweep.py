...
LSTM_SEEDS = [67, 42, 36, 2026]
RUN_SEEDS = LSTM_SEEDS
METRIC_KEYS = [
    "final_val_accuracy",
    "final_val_precision",
    "final_val_recall",
...
def write_seed_config(
    ...
    seed_config["tracking"].setdefault("mlflow", {})["run_name"] = f"lstm_w2vec_seed{seed}"

    output_dir.mkdir(parents=True, exist_ok=True)
    config_path = output_dir / f"experiment_seed{seed}.yaml"
...
def load_seed_metrics(seed: int, outputs_dir: Path = Path("outputs")) -> dict[str, float]:
    metrics_path = outputs_dir / f"lstm_w2vec_seed{seed}_metrics.yaml"
...
def run_seed_sweep(
    ...
    run_seeds = RUN_SEEDS if run_seeds is None else run_seeds
    ...
    summary_path = Path("outputs") / "lstm_w2vec_seed_summary.yaml"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run LSTM Word2Vec seed sweep.")
